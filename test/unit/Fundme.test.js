...
describe("Fundme", () => {
  ...
  describe("constructor", () => {
    it("sets the aggregator addressess correctly", async () => {
      ...
      // TODO we will check if our constructor priceFeed address is gonna be same as MockV3Aggregator becoz we are deploying it on hardhat localhost
  ...
  describe("fund", () => {
    ...
    it("update the amount funded data structure", async () => {
      ...
      const response = await fundMe.addressToAmountFunded(deployers);
      assert.equal(response.toString(), sendValue.toString());
    });

    //NOTE test to check if msg.sender(deployer) added successfully to the funders array
  ...
  describe("withdraw", () => {
    ...
    //ANCHOR test for withdrawing Eth when there are multiple Funders
    it("allows us to withdraw with multiple funders", async () => {
      ...
      // TODO make sure that the funders[] array reset properly in withdraw()
